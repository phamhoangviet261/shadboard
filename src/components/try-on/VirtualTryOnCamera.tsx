      ...
      >
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2 pointer-events-none">
          {camera.stream ? (
            <Badge
              variant="secondary"
              className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 backdrop-blur-md"
            >
            ...
            <Badge
              variant="secondary"
              className="bg-neutral-500/10 text-neutral-400 border-neutral-500/20 backdrop-blur-md"
            >
              Camera Off
            </Badge>
          )}
          ...
          {camera.stream &&
            (isDetecting ? (
              <Badge
                variant="secondary"
                className="bg-blue-500/10 text-blue-400 border-blue-500/20 backdrop-blur-md"
              >
              ...
              <Badge
                variant="secondary"
                className="bg-orange-500/10 text-orange-400 border-orange-500/20 backdrop-blur-md"
              >
        ...
        <div className="absolute top-4 right-4 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <Badge
            variant="secondary"
            className="bg-black/40 text-[10px] uppercase tracking-wider text-neutral-400 border-neutral-800 backdrop-blur-md"
          >
        ...
        {!camera.stream && !camera.isLoading && !camera.error && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-neutral-900/80 backdrop-blur-sm z-10 text-center p-6">
            ...
            <p className="text-neutral-400 max-w-xs mb-6">
              Enable your camera to see how these glasses look on you. All
          ...
          <div className="flex items-center gap-2">
            ...
            {camera.devices.length > 1 && (
              <Button
                ...
                size="sm"
                onClick={() => {
                  const currentIndex = camera.devices.findIndex(
                    (d) => d.deviceId === camera.activeDeviceId
                  )
